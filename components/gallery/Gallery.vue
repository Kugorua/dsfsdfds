<template>
  <div class="gallery" ref="intor">
    <div class="gallery__container">
      <div
        class="gallery__container__item"
        :class="{ gallery__container__scroll: isCroll }"
        v-for="(item, index) in dataIntroduce"
        :key="index"
      >
        <div class="gallery__container__item__icon">
          <div class="gallery__container__item__icon__wrap">
            <div class="gallery__container__item__icon__wrap__img"></div>
          </div>
          <div class="gallery__container__item__icon__img">
            <img :src="item.img" alt="" />
          </div>
        </div>

        <div class="gallery__container__item__infor">
          <span class="gallery__container__item__infor__number">{{
            item.number
          }}</span>
          <span class="gallery__container__item__infor__text">
            {{ item.text }}</span
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    handleScroll() {
      if (window.top.scrollY >= Number(this.$refs.intor.offsetTop)-375) {
        this.isCroll = true;
